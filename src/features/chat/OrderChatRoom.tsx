...
import { Send, Loader2, MessageSquareOff, Lock } from "lucide-react";

interface Props {
  orderId: string;
  receiverName: string;
  chatType: "merchant_customer" | "courier_merchant" | "courier_customer";
  receiverId?: string; // 🚀 SEKARANG WAJIB ADA AGAR TERKONEKSI
...
export const OrderChatRoom: React.FC<Props> = ({
  ...
  receiverName,
  chatType,
  receiverId, // 👈 Alamat tujuan pesan
  merchantId,
}) => {
  const { user } = useAuth();
  ...
  const fetchChatAndStatus = async () => {
    try {
      setLoading(true);
      // 🚀 AMBIL SEMUA CHAT DI ORDER INI DENGAN TIPE YANG SAMA
      let query = supabase
        .from("order_chats")
        .select("*")
        .eq("order_id", orderId)
      ...
      // Cek Status Order untuk Lock Chat
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chat_order_${orderId}_${chatType}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  const sendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || isChatLocked || !user?.id) return;

    const content = newMessage;
    setNewMessage("");

    // 🚀 SEKARANG KITA KIRIM DENGAN RECEIVER_ID
    const { error } = await supabase.from("order_chats").insert({
      order_id: orderId,
      sender_id: user.id,
      receiver_id: receiverId, // 👈 TUJUAN PESAN (ID TOKO / ID PEMBELI)
      message: content,
      chat_type: chatType,
    ...
    if (error) {
      ...
      setNewMessage(content); // Kembalikan teks jika gagal
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center">
            ...
            <p className="text-[10px] text-slate-400 text-center max-w-[200px] tracking-widest uppercase">
              MULAILAH OBROLAN DENGAN {receiverName}
            </p>
          </div>
        ) : (
          messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[85%] p-3 md:p-4 rounded-md text-[12px] shadow-sm font-bold normal-case tracking-normal border ${isMe ? "bg-[#008080] border-[#008080] text-white" : "bg-white border-slate-200 text-slate-800"}`}
                >
                  {msg.message}
                  <div
        ...
        {!isChatLocked ? (
          ...
          >
            <input
              ...
              placeholder={`KIRIM PESAN KE ${receiverName}...`}
              className="flex-1 bg-transparent px-3 py-2 text-[12px] md:text-[14px] font-black outline-none placeholder:text-slate-300"
            />
            <button
              type="submit"
              disabled={!newMessage.trim()}
              className="w-12 h-12 bg-[#008080] text-white rounded-md flex items-center justify-center shadow-md active:scale-90 transition-all disabled:opacity-50"
            >
              <Send size={18} />
            </button>
          </form>
        ) : (
          <div className="bg-red-50 p-4 rounded-md flex items-center justify-center gap-2 text-red-500 border border-red-200">
            <Lock size={16} />
