...
interface ChatContextType {
  ...
  unreadTotal: number;
  openChat: (participantId: string, participantName: string, role: string, orderId?: string) => void;
  closeChat: () => void;
...
export const ChatProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const unreadTotal = threads.reduce((acc, thread) => acc + thread.unreadCount, 0);

  // Helper: Open Chat or Create New if not exists
  const openChat = (participantId: string, participantName: string, role: string, orderId?: string) => {
    const existingThread = threads.find(t => t.participantId === participantId);
  ...
  const getAutoReply = (msg: string): string => {
      ...
      if (lower.includes('ready') || lower.includes('stok')) return "Stok ready kak, silakan diorder.";
      if (lower.includes('kirim') || lower.includes('sampai')) return "Sedang dalam perjalanan ya kak.";
      if (lower.includes('terima kasih') || lower.includes('makasih')) return "Sama-sama kak! Ditunggu order selanjutnya.";
