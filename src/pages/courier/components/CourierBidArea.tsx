...
interface Props {
  ...
  acceptOrder: (orderId: string) => void; // 🚀 INI DIA KABEL NAVIGASINYA!
}
...
export const CourierBidArea: React.FC<Props> = ({
  ...
  currentCoords,
  onOrderAccepted,
  acceptOrder, // 🚀 TERIMA FUNGSINYA DARI ATAS
}) => {
  ...
  const fetchNearbyOrders = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        // 🚀 FIX STATUS: Harus "READY_TO_PICKUP", bukan "SEARCHING_COURIER" (sesuai standar di sistem kita)
        .eq("status", "READY_TO_PICKUP")
        .is("courier_id", null) // Pastikan belum diambil orang lain
    ...
    <div className="w-full space-y-4 font-black uppercase tracking-tighter text-left">
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-slate-400">
          <Radar className="w-16 h-16 animate-ping mb-4 text-slate-300" />
          <p>MENCARI TUGAS DI SEKITARMU...</p>
        </div>
      ) : (
        orders.map((o) => (
          <div
          ...
          >
              ...
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-slate-800 font-bold truncate">
                    {o.merchants?.address ||
                      "LOKASI TOKO BELUM DISET DI PROFIL"}
                  </p>
                </div>
              </div>
              <div className="flex gap-3">
            ...
            <button
              ...
              // 🚀 INI DIA EKSEKUSINYA! KIRIM ID ORDER KE HOOK
              onClick={() => {
                setIsProcessing(o.id);
                acceptOrder(o.id);
              }}
              className="w-full py-4 bg-slate-900 text-white rounded-xl text-[12px] flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all disabled:opacity-50"
            >
              {isProcessing === o.id ? (
                <>
