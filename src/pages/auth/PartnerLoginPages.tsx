  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-slate-500 hover:text-teal-600 transition-colors mb-6 font-bold text-[10px] uppercase tracking-widest pl-2"
        >
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl overflow-hidden border border-gray-100">
          ...
          <div className="p-8 pt-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-1.5 text-left group">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-14 pr-6 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl text-sm font-bold text-slate-900 placeholder:text-slate-300 focus:bg-white focus:border-teal-500 outline-none transition-all shadow-inner"
              ...
              <button
                disabled={isLoading}
                className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-lg hover:bg-slate-800 active:scale-95 transition-all flex justify-center items-center gap-3 mt-4 disabled:opacity-70 group"
              >
                {isLoading ? (
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.role === "LOCAL_ADMIN") {
        ...
        await supabase.auth.signOut();
        throw new Error(
          "Portal ini khusus Admin Wilayah. Pelanggan dilarang masuk.",
        );
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-slate-500 hover:text-white transition-colors mb-6 font-bold text-[10px] uppercase tracking-widest pl-2"
        >
          <ChevronLeft size={16} /> Kembali ke Beranda
        </Link>
        ...
        <div className="bg-slate-800 rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-700">
          ...
          <div className="p-8 pt-8">
            <form onSubmit={handleAdminLogin} className="space-y-5">
              <div className="space-y-1.5 text-left group">
                <label className="text-[10px] font-black text-slate-500 uppercase ml-4 tracking-widest group-focus-within:text-blue-500 transition-colors">
                  Email Admin
                </label>
                <div className="relative">
                  ...
                  <input
                    type="email"
                    placeholder="admin@zona.com"
                    className="w-full pl-14 pr-6 py-4 bg-slate-900/50 border-2 border-slate-700 rounded-2xl text-sm font-bold text-white placeholder:text-slate-600 focus:border-blue-500 outline-none transition-all shadow-inner"
                    value={formData.email}
                    onChange={(e) =>
                      setFormData({ ...formData, email: e.target.value })
                    }
              ...
              <button
                disabled={isLoading}
                className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-lg shadow-blue-900/50 hover:bg-blue-500 active:scale-95 transition-all flex justify-center items-center gap-3 mt-4 disabled:opacity-70 group"
              >
                {isLoading ? (
            ...
            <div className="mt-8 pt-6 border-t border-slate-700/50 text-center">
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide mb-2">
                Ingin mendaftar sebagai Admin Wilayah?
              </p>
              <Link
                to="/promo/admin"
                className="inline-flex items-center gap-1 text-blue-400 font-black text-[11px] uppercase tracking-widest hover:text-blue-300 transition-colors border-b-2 border-transparent hover:border-blue-400 pb-0.5"
              >
                Ajukan Permohonan <ArrowRight size={12} />
              </Link>
            </div>
          </div>
        </div>
      ...
      <div className="w-full max-w-md relative z-10 animate-in zoom-in-95 duration-700">
        <div className="bg-black/80 backdrop-blur-xl rounded-[2rem] shadow-2xl overflow-hidden border border-purple-900/50">
          <div className="p-10 pb-0 text-center">
            ...
            <p className="text-purple-400 text-[10px] font-bold uppercase tracking-[0.3em]">
          ...
          <div className="p-10">
            <form onSubmit={handleSuperLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-purple-500 uppercase tracking-widest ml-2">
                  Secure ID
                </label>
                <input
                  type="email"
                  placeholder="root@pasarmj.id"
                  className="w-full px-6 py-4 bg-purple-900/10 border border-purple-900/50 rounded-xl text-white font-bold placeholder:text-purple-900/50 focus:border-purple-500 focus:bg-purple-900/20 focus:shadow-[0_0_20px_rgba(168,85,247,0.2)] outline-none transition-all text-center tracking-wider"
                  value={formData.email}
                  onChange={(e) =>
                    setFormData({ ...formData, email: e.target.value })
                  }
              ...
              <div className="space-y-2">
                ...
                <input
                  type="password"
                  placeholder="••••••••"
                  className="w-full px-6 py-4 bg-purple-900/10 border border-purple-900/50 rounded-xl text-white font-bold placeholder:text-purple-900/50 focus:border-purple-500 focus:bg-purple-900/20 focus:shadow-[0_0_20px_rgba(168,85,247,0.2)] outline-none transition-all text-center tracking-[0.5em]"
                  value={formData.password}
                  onChange={(e) =>
                    setFormData({ ...formData, password: e.target.value })
                  }
              ...
              <button
                disabled={isLoading}
                className="w-full py-5 bg-purple-700 text-white rounded-xl font-black uppercase text-xs tracking-[0.3em] shadow-lg shadow-purple-900/50 hover:bg-purple-600 hover:scale-[1.02] active:scale-95 transition-all flex justify-center items-center gap-3 mt-6 disabled:opacity-50"
              >
                {isLoading ? (
                  <Loader2 className="animate-spin" />
                ) : (
                  <>
                    <Zap size={16} /> INITIATE
                  </>
                )}
              </button>
            </form>
          ...
          <div className="bg-purple-900/20 p-4 text-center border-t border-purple-900/30">
            <p className="text-[9px] text-purple-600 font-mono">
