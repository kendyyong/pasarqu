...
export const PortalLoginPage = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
            ...
            <Link to="/" className="absolute top-8 left-8 flex items-center gap-2 text-slate-400 hover:text-teal-600 transition-colors text-xs font-black uppercase tracking-widest text-left z-20">
            ...
            <div className="relative z-10 w-full max-w-md flex flex-col items-center">
                ...
                <div className="w-full space-y-4">
                    ...
                    {/* BAGIAN ADMIN WILAYAH SUDAH DIHAPUS DARI SINI */}
                    ...
                    <div className="pt-10 text-center border-t border-slate-200 mt-8 w-full">
                        ...
                        <div className="flex justify-center gap-4 text-[11px] font-bold">
                            <button onClick={()=>navigate('/promo/toko')} className="text-teal-600 hover:underline">Daftar Toko</button>
                            <span className="text-slate-300">|</span>
                            <button onClick={()=>navigate('/promo/kurir')} className="text-orange-600 hover:underline">Daftar Kurir</button>
                        </div>
                    </div>
                </div>
            </div>
...
const PortalCard = ({ icon, title, desc, onClick, color }: any) => (
    ...
    >
        ...
        <div className="text-left flex-1">
            <h3 className="font-black uppercase text-slate-800 tracking-tight leading-none mb-1.5 text-sm">{title}</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide leading-tight">{desc}</p>
        </div>
