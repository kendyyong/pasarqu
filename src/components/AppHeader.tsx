  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[1000] bg-teal-600/95 backdrop-blur-md border-b border-white/10 shadow-lg transition-all duration-300">
      <div className="max-w-[1200px] mx-auto px-4 h-[70px] md:h-[80px] flex items-center gap-3 md:gap-8">
        {/* LOGO AREA - PATEN PNG & TEKS TEGAK */}

        <div
          className="flex items-center gap-3 cursor-pointer shrink-0 group"
          onClick={() => navigate("/")}
        >
          {/* Container Logo Ikon 3D */}
          ...
          <div className="relative h-10 w-10 md:h-12 md:w-12 flex items-center justify-center bg-white rounded-xl shadow-xl border border-white/20 transform group-hover:scale-110 transition-all duration-300 p-1">
            <img
              ...
              onError={(e) => {
                // Fallback jika file logo-pasarqu.png tidak ditemukan di folder public
          ...
          {/* Teks Branding - Lurus & Dinamis (Otomatis dari Database) */}
          ...
          <div className="hidden sm:block">
            ...
            {/* Menampilkan Branding Area (brandName) atau Nama Pasar asli secara otomatis */}
        ...
        {/* ACTION BUTTONS - ✅ PERBAIKAN: Jarak gap lebih rapat di Mobile (gap-1) */}
        ...
        <div className="flex items-center gap-1 md:gap-4 shrink-0">
          ...
          {/* User Profile - Hidden di mobile, muncul di desktop (md:flex) */}

          <div
            onClick={onUserClick}
            className="hidden md:flex items-center gap-2 cursor-pointer bg-white/10 p-1.5 rounded-xl border border-white/10 transition-all active:scale-95"
          >
            {user && userAvatar ? (
              <img
                src={userAvatar}
